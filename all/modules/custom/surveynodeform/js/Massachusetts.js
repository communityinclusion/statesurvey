(function($) {
 ...
 Drupal.behaviors.remState = {
   ...
   $('fieldset.field-group-tab').each(function(i, elem) {
   ...
   surveynodeformInitialMdMinWage(hours,wages);
    ...
    $(".form-type-checkbox input").change(function() {
      if($(this).is(':checked')) {var fieldID =$(this).closest('fieldset.field-group-tab').prop('id');
     if($('#' + fieldID).hasClass('showQues'))  $('#' + fieldID).removeClass('showQues');
      ...
      $('#' + fieldID + ' input.form-radio').each(function(i,el){
   ...
   Drupal.behaviors.surveynodeformMdGrpIntegJob = {
     attach: function (context, settings) {
       surveynodeformMdMinWage('#edit-field-grp-integ-hrs-und-0-value', '#edit-field-grp-integ-gross-wages-und-0-value');
   ...
   function surveynodeformMdMinWage(hours, wages) {
     $(hours + ', ' + wages).change(function() {
       var hasValues = false;
       if (($(wages).val() && $(wages).val() != '') && ($(hours).val() && $(hours).val() != '')) {
         hasValues = true;
       }
       var hourly_rate = hasValues ? $(wages).val() / $(hours).val() : null;
       if ((hourly_rate < Drupal.settings.Surveyconfig.mamin) && (hasValues)) {
          if(!$(wages).closest('fieldset').hasClass('showQues')) $(wages).closest('fieldset').addClass('showQues');
       //  $(wages).val('');
         alert('Are you sure about the wages and hours you entered? Hours/wages are less than the Mass. minimum wage of  $' + Drupal.settings.Surveyconfig.mamin + '/hr.');
       }
       else {
         surveynodeformClearMin(hours);
         if($(wages).closest('fieldset').hasClass('showQues')) $(wages).closest('fieldset').removeClass('showQues');

       }
     });
   }
    function surveynodeformClearMin(hours) {

     var minChoice = hours == '#edit-field-indv-comp-hrs-und-0-value' ? 'edit-field-indv-emp-less-und' :(hours == '#edit-field-grp-integ-hrs-und-0-value' ? 'edit-field-grp-sup-less-und' : 'nostate');
     var minText = hours == '#edit-field-indv-comp-hrs-und-0-value' ?  'edit-field-indv-emp-less-other-und-0-value' : (hours == '#edit-field-grp-integ-hrs-und-0-value' ? 'edit-field-grp-sup-less-other-und-0-value' : 'nostate');
     var minRadio = hours == '#edit-field-indv-comp-hrs-und-0-value' ?  'field_indv_emp_less[und]' : (hours == '#edit-field-grp-integ-hrs-und-0-value' ? 'field_grp_sup_less[und]' : 'nostate');
     $( '#' + minChoice + ' input.form-radio').each(function ()
      {
       $(this).removeAttr("checked");

      });
       $('input#' + minChoice + '-2').attr('checked', true);
       $('input[name="' + minRadio + '"]').val([2]);
   ...
   if(!$('fieldset.showQues input#edit-field-indv-emp-less-und-1').is(':checked')) {
    if(!$('input#edit-field-indv-emp-less-other-und-0-value').hasClass('notReq')) $('input#edit-field-indv-emp-less-other-und-0-value').addClass('notReq');} else {
      if($('fieldset.showQues input#edit-field-indv-emp-less-other-und-0-value').hasClass('notReq')) $('input#edit-field-indv-emp-less-other-und-0-value').removeClass('notReq');
   ...
   $('div.field-widget-options-buttons:visible').each(function(i, el) {
         ...
         if(!$(el).hasClass('fieldReq') && !$(el).hasClass('field-type-list-integer') &&  !$(el).parents('.checkGroup').length > 0) { $(el).addClass('fieldReq');}
   ...
   $('div.vertical-tabs-panes > fieldset.active div input.form-radio').each(function(i, elem) {
   if( $(elem).is(":visible") && !$(elem).closest('div.form-wrapper').hasClass('visDiv')) {
   $(elem).closest('div.form-wrapper').addClass('visDiv');
 } else { if ( !$(elem).is(":visible") && $(elem).closest('div.form-wrapper').hasClass('visDiv')) {
    $(elem).closest('divform-wrapper').removeClass('visDiv');
    }
...
$('div.vertical-tabs-panes > fieldset.active .visDiv .form-radios').each(function(i, elem) {

  var radiosid = $(elem).attr("id");

  if(!$('#' + radiosid + '  label:visible > input.form-radio:checked').val())
  {// alert('Nothing is checked!');
  //countempty += 1;
    if (!$(elem).closest('div.form-wrapper').hasClass('redLine')){ $(elem).closest('div.form-wrapper').addClass('redLine');}
  } else {// alert('One of the radio buttons is checked!');
     if ($(elem).closest('div.form-wrapper').hasClass('redLine')){ $(elem).closest('div.form-wrapper').removeClass('redLine');}
}
