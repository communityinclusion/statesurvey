...
// causing doubled check and radio inputs
/*   if (isset($element['#type'])) {
 ...
 function statedata_boot_preprocess_views_view(&$vars) {
    $view = $vars['view'];

     // Add Javascript when the view with the name news is shown and the display id is your-display-id.
     if ($view->name == 'provider_individuals' && $view->current_display == 'page_1') {
        drupal_add_js("(function($){ $(document).ready(function() {
          $('.editIndData').tooltip();
        $('.editInd').tooltip();
      $('.deleteInd').tooltip();
    $('.addInd').tooltip();
  $('.completPop').tooltip(); });})(jQuery);", 'inline');
     }
   }
